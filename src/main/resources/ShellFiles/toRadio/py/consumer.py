...
def ToRadio(news_rpc_obj):
    ...
    cmd = "ffmpeg -re -v quiet -stats -i https://rfa.toloka.media/store/audio/"
    cmd += news_rpc_obj["newsStoreUUID"]
    cmd += " -f mp3 https://"+ news_rpc_obj["username"] +":"+ decrypted_message.decode() +"@rfa.toloka.media:" + news_rpc_obj["mainport"]
    cmd+= "/main" # + news_rpc_obj["mainpoint"]

    print(cmd)
    process = subprocess.Popen(['bash',cmd])

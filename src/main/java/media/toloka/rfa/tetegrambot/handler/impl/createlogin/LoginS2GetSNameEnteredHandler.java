...
@Profile("Telegram")
@Component
public class LoginS2GetSNameEnteredHandler extends UserRequestHandler {

    private final TelegramService telegramService;
    private final KeyboardHelper keyboardHelper;
    private final UserSessionService userSessionService;

    public LoginS2GetSNameEnteredHandler(TelegramService telegramService, KeyboardHelper keyboardHelper, UserSessionService userSessionService) {
...
//        return isTextMessage(userRequest.getUpdate(), "ееее"+BTN_TO_RFA_REGISTERS);
        return isTextMessage(userRequest.getUpdate())
                && ConversationState.WAITING_LOGIN_SNAME.equals(userRequest.getUserSession().getState());
...
//        );
    ...
    public void handle(UserRequest userRequest) {
        ...
        session.setUserSName (userRequest.getUpdate().getMessage().getText());

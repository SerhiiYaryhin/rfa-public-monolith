...
@Profile("Telegram")
@Component
public class LoginS1GetFNameEnteredHandler extends UserRequestHandler {

    private final TelegramService telegramService;
    private final KeyboardHelper keyboardHelper;
    private final UserSessionService userSessionService;

    public LoginS1GetFNameEnteredHandler(TelegramService telegramService, KeyboardHelper keyboardHelper, UserSessionService userSessionService) {
...
//        return isTextMessage(userRequest.getUpdate(), "ееее"+BTN_TO_RFA_REGISTERS);
        return isTextMessage(userRequest.getUpdate())
                && ConversationState.WAITING_LOGIN_FNAME.equals(userRequest.getUserSession().getState());
...
//        );
    ...
    public void handle(UserRequest userRequest) {
        ...
        session.setUserFName (userRequest.getUpdate().getMessage().getText());
        // Встановлюємо стан для наступного кроку
        session.setState(ConversationState.WAITING_LOGIN_SNAME); //Для наступного кроку
        // Зберігаємо в статусі користувача сеанс та користувача
        userSessionService.saveSession(userRequest.getChatId(), session);
        // Повідомлення про наступний крок
        ReplyKeyboardMarkup replyKeyboardMarkup = keyboardHelper.buildMenuWithCancel();
        telegramService.sendMessage(userRequest.getChatId(),"Будь ласка, введіть Ваше призвище.",
                replyKeyboardMarkup);

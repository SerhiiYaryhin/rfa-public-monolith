...
public class LoginS4GetPasswordEnteredHandler extends UserRequestHandler {
    ...
    public LoginS4GetPasswordEnteredHandler(TelegramService telegramService, KeyboardHelper keyboardHelper, UserSessionService userSessionService) {
...
//                && ConversationState.WAITING_LOGIN_PASSWORD.equals(userRequest.getUserSession().getState());
    }
    ...
    public void handle(UserRequest userRequest) {
        ...
        session.setUserPassword (userRequest.getUpdate().getMessage().getText());
        // Встановлюємо стан для наступного кроку
        session.setState(ConversationState.WAITING_LOGIN_EMAIL); //Для наступного кроку
        // Зберігаємо в статусі користувача сеанс та користувача
        userSessionService.saveSession(userRequest.getChatId(), session);
        // Повідомлення про наступний крок
        ReplyKeyboardMarkup replyKeyboardMarkup = keyboardHelper.buildMenuWithCancel();
        telegramService.sendMessage(userRequest.getChatId(),"Будь ласка, введіть вашу пошту. Ми відправимо Вам листа для активації аккаунту.",
                replyKeyboardMarkup);
